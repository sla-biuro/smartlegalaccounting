<template>
  <header class="sticky top-0 z-50">
    <div class="relative backdrop-blur-xl bg-gradient-to-r from-brandBlue/10 via-brandLightBlue/5 to-brandYellow/10 shadow-xl">
      <!-- Background pattern -->
      <div class="absolute inset-0 bg-gradient-to-r from-white/80 via-white/70 to-white/60 dark:from-gray-900/80 dark:via-gray-900/70 dark:to-gray-900/60"></div>
        ...
        <div class="h-16 flex items-center justify-between">
          ...
          <nav class="hidden md:flex items-center gap-4">
            <template v-for="(item, idx) in nav" :key="idx">
              <NuxtLink v-if="!item.emphasis" class="text-sm text-muted hover:text-neutral transition-colors" :to="item.to">{{ item.label }}</NuxtLink>
              <NuxtLink v-else class="ml-2 btn btn-gradient py-2" :to="item.to">{{ item.label }}</NuxtLink>
            </template>
      ...
      <div class="pointer-events-none absolute inset-x-0 -bottom-px h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent"></div>
    </div>
    <!-- Mobile menu panel -->
      ...
      <div v-if="open" class="md:hidden bg-white/80 dark:bg-gray-900/80 border-t border-gray-200/50 dark:border-gray-700/40">
        <div class="px-4 py-3 space-y-2">
          <template v-for="(item, idx) in nav" :key="'m-'+idx">
            ...
            <NuxtLink v-else class="block text-base btn btn-gradient py-2" :to="item.to" @click="open = false">{{ item.label }}</NuxtLink>
          </template>
